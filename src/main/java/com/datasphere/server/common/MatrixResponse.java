...
public class MatrixResponse<R, C> implements Serializable {
  ...
  /**
   * 총 Values List, addMinMax Temporary space for options (treemap/heatmap Utilize in)
   */
  @JsonIgnore
  List<C> values;
  ...
  /**
   * Scatter Data conversion for chart representation
   */
  public MatrixResponse<R, C> reshapeForScatter(String columnDelimeter) {
    ...
    for (int i = 0; i < rowSize; i++) {
      ...
      for (int j = 0; j < columns.size(); j++) {
        ...
        // MinMax Because the values are measured on the y axis, they are treated separately.
        if (j == 1) {
  ...
  /**
   * Heatmap Data conversion for chart representation
   */
  public MatrixResponse<R, C> reshapeForHeatmap(String columnDelimeter) {
    List<C> valueForMinMax = Lists.newArrayList(); // MinMax Value for processing
    List<R> newRows = Lists.newArrayList();
    List<Column<C>> newColumns = Lists.newArrayList();
    int rowSize = rows.size();
    ...
    for (Integer i = 0; i < rowSize; i++) {
      ...
      for (Integer j = 0; j < columnSize; j++) {
        ...
        // Add row / column index value and Value
        List<C> newValue = Lists.newArrayList();
        newValue.add((C) i);
  ...
  /**
   * Grid Data conversion for presentation
   */
  ...
  /**
   * Boxplot Data conversion for chart representation
   */
  ...
  /**
   * Pie Data conversion for chart representation
   */
  public MatrixResponse reshapeForPie() {
    ...
    for (Column<C> column : columns) {
      ...
      for (int i = 0; i < originalValues.size(); i++) {
        ...
        // Pivoting Exclude if no result value
        if (value == null) {
          continue;
        }
  ...
  /**
   * Wordcloud Data conversion for chart representation
   */
  ...
  public MatrixResponse reshapeForTreeMap(String columnDelimeter, boolean setPercentage) {
    ...
    // Pass empty if there is no value
  ...
  /**
   * Remove the last measure field and process only the dimension field by dividing it into column delimiters
   */
  ...
  public void addMinMax() {
    ...
    for (int i = 0; i < values.size(); i++) {
      ...
      Double target = null;
      if (obj instanceof NameValuePair) {  // TreeMap Consider the NameValuePair use case in
        Object valueInPair = ((NameValuePair) obj).getValue();
        if (valueInPair instanceof Number) {
          target = ((Number) valueInPair).doubleValue();
        }
      } else if (obj instanceof Number) {  // Number Calculate only if
        target = ((Number) obj).doubleValue();
      } else if (obj instanceof NumericNode) {  // NumericNode Calculate only if
